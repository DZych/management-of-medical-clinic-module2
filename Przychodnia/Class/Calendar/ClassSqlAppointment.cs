...
namespace Przychodnia.Class.Calendar
{
    public class ClassSqlAppointment
    {
        ...
        {
            string querry = "SELECT dbo.tbl_Appointment.Appointment_id, [dbo].[tbl_Appointment].Start_time, [dbo].[tbl_Patient].Name, [dbo].[tbl_Patient].Surname, [dbo].[tbl_Patient].Personal_identity_number, [dbo].[tbl_Appointment].Topic  FROM [dbo].[tbl_Appointment], [dbo].[tbl_Patient] WHERE Term_id = ( SELECT tbl_Term.Term_id FROM [dbo].[tbl_Term] WHERE tbl_Term.Date = '"+ date +"' AND [dbo].[tbl_Term].Doctor_id = " + ClassLoggedDoctor.Doctor_Id + ") AND [dbo].[tbl_Appointment].Patient_id = [dbo].[tbl_Patient].Patient_id";
            SqlDataReader dr = ClassQuerry.ExecuteQuerry(querry);
